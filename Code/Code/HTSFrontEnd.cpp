...
cl_uint CFrontEnd::uiSubmitReq(TFid tFid, CRequest& cReq, TEvent& tEvent)
{
  ...
  //if it is a blocking request, wait till it is serviced.
  if(cReq.uiFlags & HTS_REQ_BLOCKING)
    {
      ResetEvent(tFid->hBlockEvent);
      WaitForSingleObject(tFid->hBlockEvent, INFINITE);
    }

  return HTS_OK;
}

cl_uint CFrontEnd::uiGetStatus(TFid tFid, TEvent tEvent, cl_uint& uiRStatus)
{
  ...
  if(uiFlags & HTS_REQ_COMPLETED)
    {
      (tFid->pThreadRequest[uiReqId]).uiFlags = 0;
      uiRStatus = (tFid->pThreadRequest[uiReqId]).uiStatus;      
...
UINT CFrontEnd::uiGetThreadCount()
{
  return uiTidCount;
}
